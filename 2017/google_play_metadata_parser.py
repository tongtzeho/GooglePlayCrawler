...
def get_app_basic_info(data):
	...
	matcher = re.findall('<span itemprop="genre">.*?</span>', data, re.S)
	if len(matcher):
		categoryall = ""
...
def get_similar_apps_link(data):
	link = None
	matcher = re.findall('href="/store/apps/collection/similar_apps.*?"', data)
	if len(matcher):
		link = matcher[0].replace('href="/', "https://play.google.com/").replace('"', "")
	return link

def get_similar_apps(data):
	urls = []
	urls_set = set()
	...
	for url in matcher:
		...
		if full_url.startswith("https://play.google.com/store/apps/details?id="):
			...
			if not new_pkg in urls_set:
				urls.append(new_pkg)
				urls_set.add(new_pkg)
	return urls
